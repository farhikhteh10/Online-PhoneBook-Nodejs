...
if (!fs.existsSync(configPath)) {
    console.error('\x1b[31m%s\x1b[0m', 'Error: config.json not found. Please create it with your database credentials.');
    process.exit(1);
}
...
// --- 2. Initial Data ---
// This data is copied from your application's initial state.
...
const initialUser = {
    username: 'admin',
    // IMPORTANT: This hash is for the password "admin". 
    // You should replace this with a securely generated hash in a real production environment.
...
const createSchemaSQL = `
    ...
    CREATE TABLE IF NOT EXISTS personnel (
        id SERIAL PRIMARY KEY,
        personnel_code VARCHAR(20) UNIQUE NOT NULL,
        persian_name VARCHAR(255) NOT NULL,
        english_name VARCHAR(255) NOT NULL,
        voip_number VARCHAR(20) UNIQUE NOT NULL,
...
async function setupDatabase() {
    ...
    try {
        ...
        console.log('\x1b[32m%s\x1b[0m', 'Schema created successfully (or already exists).');

        // Step 2: Clear existing data to prevent duplicates on re-run
        console.log('Clearing existing data...');
        await client.query('TRUNCATE personnel, projects, departments, positions, users, app_settings RESTART IDENTITY CASCADE;');

        // Step 3: Populate lookup tables (projects, departments, positions)
        console.log('Populating lookup tables...');
        const projectsMap = new Map();
        const departmentsMap = new Map();
        const positionsMap = new Map();

        const uniqueProjects = [...new Set(initialPersonnelData.map(p => p.project))];
        for (const name of uniqueProjects) {
            const res = await client.query('INSERT INTO projects (name) VALUES ($1) RETURNING id', [name]);
            projectsMap.set(name, res.rows[0].id);
        }

        const uniqueDepartments = [...new Set(initialPersonnelData.map(p => p.department))];
        for (const name of uniqueDepartments) {
            const res = await client.query('INSERT INTO departments (name) VALUES ($1) RETURNING id', [name]);
            departmentsMap.set(name, res.rows[0].id);
        }

        const uniquePositions = [...new Set(initialPersonnelData.map(p => p.position))];
        for (const name of uniquePositions) {
            const res = await client.query('INSERT INTO positions (name) VALUES ($1) RETURNING id', [name]);
            positionsMap.set(name, res.rows[0].id);
        }
        console.log('\x1b[32m%s\x1b[0m', 'Lookup tables populated.');

        // Step 4: Populate personnel table
        console.log('Populating personnel table...');
        for (const person of initialPersonnelData) {
            await client.query(
                `INSERT INTO personnel (personnel_code, persian_name, english_name, voip_number, project_id, department_id, position_id)
                ...
                [
                    ...
                    person.englishName,
                    person.voipNumber,
                    projectsMap.get(person.project),
                    departmentsMap.get(person.department),
                    positionsMap.get(person.position)
                ]
            );
        }
        console.log('\x1b[32m%s\x1b[0m', `${initialPersonnelData.length} personnel records inserted.`);

        // Step 5: Populate users and app_settings
        console.log('Populating users and settings...');
        await client.query('INSERT INTO users (username, password_hash) VALUES ($1, $2)', [initialUser.username, initialUser.passwordHash]);
        for (const [key, value] of Object.entries(initialAppSettings)) {
            await client.query('INSERT INTO app_settings (setting_key, setting_value) VALUES ($1, $2)', [key, value]);
        }
        console.log('\x1b[32m%s\x1b[0m', 'Default user and app settings inserted.');
        
        console.log('\n\x1b[1m\x1b[32m%s\x1b[0m', 'Database setup completed successfully!');

    } catch (err) {
        console.error('\x1b[31m%s\x1b[0m', 'An error occurred during database setup:', err);
    } finally {
        await client.end();
